...
void TeleopNode::timerCallback() {
  ...
  cmd_msg.tool_position = std::max(-100.0, std::min(joy_tool_height_state_, 100.0));
...
void TeleopNode::joyCallback(const sensor_msgs::msg::Joy::SharedPtr msg) {
  ...
  if (joy_tool_pressed) {
    if (!msg->axes[joy_raise_tool_i_] && msg->axes[joy_lower_tool_i_]) joy_tool_pressed = false;
  } else {
    //If raise bumper pressed, then raise
    if (msg->axes[joy_raise_tool_i_]) joy_tool_height_state_++;
    //If lower bumper pressed, then lower
    if (msg->axes[joy_lower_tool_i_]) joy_tool_height_state--;    
