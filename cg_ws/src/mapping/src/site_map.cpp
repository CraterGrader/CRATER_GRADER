...
SiteMap::SiteMap(size_t cHeight, size_t cWidth, float cResolution,
                 float xTransform, float yTransform, float unseenGridHeight,
                 float incomingPointVariance, float cellStartingVariance,
  ...
  varianceMap_.resize(
      height_ * width_,
      CellBayes(unseenGridHeight, cellStartingVariance, minCellVariance));
...
void SiteMap::binPts(std::vector<mapPoint> rawPts) {
  ...
  for (size_t i = 0; i < rawPts.size(); ++i) {
    float x_pos_site_map_frame =
        cg::mapping::convertMaptoSiteMapFrame(rawPts[i].x, xTransform_);
    float y_pos_site_map_frame =
        cg::mapping::convertMaptoSiteMapFrame(rawPts[i].y, yTransform_);
    discretePoints[i].x = cg::mapping::pointToDiscreteCoord(
        x_pos_site_map_frame, getResolution());
    discretePoints[i].y = cg::mapping::pointToDiscreteCoord(
        y_pos_site_map_frame, getResolution());
    discretePoints[i].z = rawPts[i].z - plane_offset_;
  }

  // use postProcess method
  std::vector<cg::mapping::indexPoint> processedPts =
      postProcess(discretePoints);

  // update view 2, filtermap
  for (size_t i = 0; i < processedPts.size(); i++) {
    size_t index = cg::mapping::discreteCoordsToCellIndex(
        processedPts[i].x, processedPts[i].y, getWidth());
    bufferMap_[index].addPoint(processedPts[i]);
    seenPointsMap_[index] = 1;
  }
}

std::vector<cg::mapping::indexPoint>
SiteMap::postProcess(std::vector<cg::mapping::indexPoint> ptsCheck) {
  // out of bounds check
  ...
  for (size_t i = 0; i < ptsCheck.size(); i++) {
    if (!cg::mapping::indexInMap(ptsCheck[i].x, ptsCheck[i].y, getWidth(),
                                 getHeight())) {
      badCount++;
...
void SiteMap::updateMapCoverage() {
  size_t sum_of_elems = static_cast<size_t>(
      std::reduce(seenPointsMap_.begin(), seenPointsMap_.end()));
  if (getNcells() == sum_of_elems) {
    siteMapFull_ = true;
  } else {
    siteMapFull_ = false;
  }
  siteMapCoverageRatio_ =
      static_cast<float>(sum_of_elems) / static_cast<float>(getNcells());
}
