...
SiteMapNode::SiteMapNode() : Node("site_map_node") {
  ...
  viz_timer_ =
      this->create_wall_timer(std::chrono::milliseconds(100),
                              std::bind(&SiteMapNode::map_viz_callback, this));

  // Initialize services
  site_map_server_ = this->create_service<cg_msgs::srv::SiteMap>(
      "site_map_server",
      std::bind(&SiteMapNode::sendSiteMap, this, std::placeholders::_1,
                std::placeholders::_2));
  save_map_server_ = this->create_service<cg_msgs::srv::SaveMap>(
      "save_map_server",
      std::bind(&SiteMapNode::saveMap, this, std::placeholders::_1,
                std::placeholders::_2));
  ...
  this->declare_parameter<std::string>(
      "load_height_map_filepath",
      "/root/CRATER_GRADER/cg_ws/src/mapping/config/"
      "half_auton_half_manual_exploration.csv");
  ...
  this->get_parameter("load_height_map_from_filepath",
                      load_height_map_from_filepath);
  this->declare_parameter<bool>("static_map", false);
  this->get_parameter("static_map", static_map_);

  cg::mapping::SiteMap temp(
      height_, width_, resolution_, xTransform_, yTransform_, unseenGridHeight_,
      incomingPointVariance_, cellStartingVariance_, minCellVariance_);
  siteMap_ = temp;
  std::cout << "Constructed SiteMap Objects" << std::endl;

  // Load map from file
  if (load_height_map_from_filepath) {
    bool load_height_map_initialized =
        fileMap_.load_map_from_file(load_height_map_filepath);
    ...
    if ((fileMap_.getHeight() != siteMap_.getHeight()) ||
      ...
      RCLCPP_FATAL(
          ...
          siteMap_.getHeight(), siteMap_.getWidth(), siteMap_.getResolution(),
          siteMap_.getHeightMap().size(), fileMap_.getHeight(),
          fileMap_.getWidth(), fileMap_.getResolution(),
          fileMap_.getCellData().size());
      rclcpp::shutdown();
    }
    ...
    std::vector<int> full_coverage(siteMap_.getHeight() * siteMap_.getHeight(),
                                   1);
  ...
  new_points_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
      "/terrain/filtered", 1,
      std::bind(&SiteMapNode::new_pts_callback, this, std::placeholders::_1));
  visualization_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>(
      "/viz/mapping/site_map_viz", 1);
  visualization_seen_map_pub_ =
      this->create_publisher<sensor_msgs::msg::PointCloud2>(
          "/viz/mapping/site_map_seen_viz", 1);
  visualization_variance_map_pub_ =
      this->create_publisher<sensor_msgs::msg::PointCloud2>(
          "/viz/mapping/site_map_variance", 1);
...
void SiteMapNode::map_viz_callback() {
  ...
  for (int col = 0; col < static_cast<int>(siteMap_.getWidth()); col++) {
    for (int row = 0; row < static_cast<int>(siteMap_.getHeight()); row++) {
      ...
      newPoint.x =
          (row * siteMap_.getResolution()) + siteMap_.getXTransform() +
          (siteMap_.getResolution() / 2); // cell x-coordinate in map frame
      newPoint.y =
          col * siteMap_.getResolution() + siteMap_.getYTransform() +
          (siteMap_.getResolution() / 2); // cell y-coordinate in map frame
  ...
  for (int col = 0; col < static_cast<int>(siteMap_.getWidth()); col++) {
    for (int row = 0; row < static_cast<int>(siteMap_.getHeight()); row++) {
      ...
      newPoint.z = static_cast<float>(
          tempMapSeen[iterator1]); // cell height in map frame
...
void SiteMapNode::new_pts_callback(
...
void SiteMapNode::sendSiteMap(cg_msgs::srv::SiteMap::Request::SharedPtr req,
                              cg_msgs::srv::SiteMap::Response::SharedPtr res) {
  (void)req; // No request input for cg_msgs/srv/SiteMap.srv, but service needs
             // both Request and Response args so just "touch" the request to
...
void SiteMapNode::saveMap(cg_msgs::srv::SaveMap::Request::SharedPtr req,
                          cg_msgs::srv::SaveMap::Response::SharedPtr res) {
  // Update map parameters and data
  fileMap_.updateDimensions(siteMap_.getHeight(), siteMap_.getWidth(),
                            siteMap_.getResolution());
