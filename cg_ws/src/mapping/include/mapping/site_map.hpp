...
public:
  // constructor
  SiteMap();
  SiteMap(size_t cHeight, size_t cWidth, float cResolution, float xTransform,
          float yTransform, float unseenGridHeight, float incomingPointVariance,
          float cellStartingVariance, float minCellVariance);
  ...
  postProcess(std::vector<cg::mapping::indexPoint>
                  ptsCheck); // method to do outlier rejection on pts
...
private:
  ...
  float resolution_ = 1.0; // the "resolution" in meters which links the map
                           // dimensions to its real world size
