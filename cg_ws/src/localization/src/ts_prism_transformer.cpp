...
TSPrismTransformer::TSPrismTransformer() : Node("ts_prism_transformer") {
  ...
  this->declare_parameter<float>("linear_translation.prism_offset_y",
                                 -0.020133);
  ...
  ts_prism_subscription_ =
      this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
          "/total_station_prism", pub_freq_,
          std::bind(&TSPrismTransformer::ts_prism_callback, this, _1));

  imu_subscription_ = this->create_subscription<sensor_msgs::msg::Imu>(
      "/imu/data/base_link", pub_freq_,
      std::bind(&TSPrismTransformer::imu_callback, this, _1));

  transformed_ts_prism_pub_ =
      this->create_publisher<geometry_msgs::msg::PoseWithCovarianceStamped>(
          "/transformed_total_station_prism", 1);

  bearing_subscription_ =
      this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
          "/bearing", pub_freq_,
          std::bind(&TSPrismTransformer::bearing_callback, this, _1));
}

void TSPrismTransformer::bearing_callback(
...
void TSPrismTransformer::ts_prism_callback(
    const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr prism_msg) {
  updated_pose_ = *prism_msg;
  if (got_imu && got_bearing_) {
    // Obtain yaw angle from bearing and roll/pitch from IMU in quaternion forms
    tf2::Quaternion bearing_q(latest_bearing_.pose.pose.orientation.x,
                              latest_bearing_.pose.pose.orientation.y,
                              latest_bearing_.pose.pose.orientation.z,
                              latest_bearing_.pose.pose.orientation.w);
    tf2::Quaternion imu_q(imu_last.orientation.x, imu_last.orientation.y,
                          imu_last.orientation.z, imu_last.orientation.w);
    ...
    // Compute 3DOF orientation from map to base_link, based on raw IMU and
    ...
    Eigen::Quaternion<double> map_to_base_link_rotation(
        map_to_base_link_q.w(), map_to_base_link_q.x(), map_to_base_link_q.y(),
    ...
    Eigen::Vector3d rotated_ts_prism_to_base_link_translation =
        map_to_base_link_rotation * ts_prism_to_base_link_translation;

    // Get position of TS prism in map frame, from raw TS measurement
    Eigen::Vector3d map_to_ts_prism_translation(
        updated_pose_.pose.pose.position.x, updated_pose_.pose.pose.position.y,
        updated_pose_.pose.pose.position.z);

    // Get position of base_link in map frame by translating position of TS
    // prism
    Eigen::Vector3d map_to_base_link_translation =
        map_to_ts_prism_translation + rotated_ts_prism_to_base_link_translation;
...
void TSPrismTransformer::imu_callback(
