...
void GetWorksystemTrajectory::runStateMultiGoal(
  ...
  for (cg_msgs::msg::Pose2D goal_pose : current_goal_poses_) {
    ...
    kinematic_planner_.generatePath(path, start_pose, goal_pose,
                                    current_height_map_);
...
void GetWorksystemTrajectory::runState(const bool worksystem_enabled,
                                       bool &updated_trajectory,
                                       bool &calculated_trajectory) {
  std::cout << "GET_WORKSYSTEM_TRAJECTORY" << std::endl;

  if (worksystem_enabled && updated_trajectory) {
    // Reset state flags for next trajectory iteration, if sent and enabled
    // correctly
    updated_trajectory = false;
    calculated_trajectory = false;

    // Update shared current state and the precursing signal if trajectory was
    // sent and worksystem is now enabled
    pre_signal_ = Signal::FOLLOW_TRAJECTORY;
    curr_state_l0_ = StateL0::FOLLOWING_TRAJECTORY;
  }
}
