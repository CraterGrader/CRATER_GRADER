...
namespace planning {
  ...
  BehaviorExecutive::BehaviorExecutive() : Node("behavior_executive_node")
  {
    ...
    this->get_parameter<double>("constant_velocity", constant_velocity_);
    this->declare_parameter<double>("high_constant_velocity", 100.0);
    this->get_parameter<double>("high_constant_velocity", high_constant_velocity_);
...
void BehaviorExecutive::fsmTimerCallback()
{
  ...
  case cg::planning::FSM::StateL0::GET_WORKSYSTEM_TRAJECTORY: {
    ...
    if (!calculated_trajectory_) {
      ...
      if (fsm_.getCurrStateL1() == FSM::StateL1::TRANSPORT && current_goal_poses_.size() == 1) {
