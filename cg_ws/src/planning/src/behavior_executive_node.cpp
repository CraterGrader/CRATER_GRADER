...
BehaviorExecutive::BehaviorExecutive() : Node("behavior_executive_node") {
  ...
  viz_path_pub_ = this->create_publisher<nav_msgs::msg::Path>(
      "/viz/planning/current_path", 1);
  viz_goals_pub_ = this->create_publisher<geometry_msgs::msg::PoseArray>(
      "/viz/planning/current_goals", 1);
  viz_state_l1_goals_pub_ =
      this->create_publisher<geometry_msgs::msg::PoseArray>(
          "/viz/planning/all_state_l1_goals", 1);
  viz_agent_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(
      "/viz/planning/current_agent", 1);
  viz_curr_goal_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>(
      "/viz/planning/current_goal", 1);

  global_robot_state_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
      "/odometry/filtered/ekf_global_node", 1,
      std::bind(&BehaviorExecutive::globalRobotStateCallback, this,
                std::placeholders::_1));
  odom_robot_state_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
      "/odometry/filtered/ekf_odom_node", 1,
      std::bind(&BehaviorExecutive::odomRobotStateCallback, this,
                std::placeholders::_1));

  /* Initialize services */
  // Create reentrant callback group for service call in timer:
  // https://docs.ros.org/en/galactic/How-To-Guides/Using-callback-groups.html
  site_map_client_group_ =
      this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
  update_trajectory_client_group_ =
      this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
  enable_worksystem_client_group_ =
      this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
  fsm_timer_cb_group_ =
      this->create_callback_group(rclcpp::CallbackGroupType::MutuallyExclusive);
  viz_timer_cb_group_ =
  ...
  site_map_client_ = this->create_client<cg_msgs::srv::SiteMap>(
      "site_map_server", rmw_qos_profile_services_default,
      site_map_client_group_);
  update_trajectory_client_ =
      this->create_client<cg_msgs::srv::UpdateTrajectory>(
          "update_trajectory_server", rmw_qos_profile_services_default,
          update_trajectory_client_group_);
  enable_worksystem_client_ =
      this->create_client<cg_msgs::srv::EnableWorksystem>(
          "enable_worksystem_server", rmw_qos_profile_services_default,
          enable_worksystem_client_group_);
  ...
  this->get_parameter("service_response_timeout_ms",
                      service_response_timeout_ms_);
  ...
  fsm_timer_ = this->create_wall_timer(
      std::chrono::milliseconds(fsm_timer_callback_ms_),
      std::bind(&BehaviorExecutive::fsmTimerCallback, this),
      fsm_timer_cb_group_);
  viz_timer_ = this->create_wall_timer(
      std::chrono::milliseconds(viz_timer_callback_ms_),
      std::bind(&BehaviorExecutive::vizTimerCallback, this),
      viz_timer_cb_group_);
  ...
  this->declare_parameter<std::string>("design_topo_filepath",
                                       "/root/CRATER_GRADER/cg_ws/src/planning/"
                                       "config/50x50_zeros_height_map.csv");
  ...
  this->get_parameter("goal_pose_distance_threshold",
  ...
  this->get_parameter("trajectory_heuristic_epsilon",
  ...
  this->get_parameter("pose_equality_scalar_iteration",
                      pose_equality_scalar_iteration);

  // Exploration planner
  double exploration_min_dist_from_map_boundary;
  this->declare_parameter<double>("exploration_min_dist_from_map_boundary",
                                  0.5);
  this->get_parameter("exploration_min_dist_from_map_boundary",
                      exploration_min_dist_from_map_boundary);
  ...
  exploration_planner_ = std::make_unique<ExplorationPlanner>(
      ExplorationPlanner(exploration_min_dist_from_map_boundary));
  kinematic_planner_ = std::make_unique<KinematicPlanner>(KinematicPlanner(
      goal_pose_distance_threshold, goal_pose_yaw_threshold, turn_radii_min,
      turn_radii_max, turn_radii_resolution, max_trajectory_length,
      trajectory_resolution, pose_position_equality_threshold,
      pose_yaw_equality_threshold, topography_weight,
      trajectory_heuristic_epsilon, max_pose_equality_scalar,
      pose_equality_scalar_iteration));
  tool_planner_ = std::make_unique<ToolPlanner>(
      ToolPlanner(design_blade_height, raised_blade_height));
  velocity_planner_ =
      std::make_unique<VelocityPlanner>(VelocityPlanner(constant_velocity));
  ...
  this->get_parameter("thresh_max_assignment_distance",
                      thresh_max_assignment_distance_);
  ...
  bool design_height_map_initialized =
  ...
  this->get_parameter("topology_equality_threshold",
                      topology_equality_threshold_);
  ...
  if ((design_height_map_.getHeight() != current_height_map_.getHeight()) ||
      ...
      (design_height_map_.getResolution() !=
       current_height_map_.getResolution())) {
    RCLCPP_FATAL(
        this->get_logger(),
        "Map dimensions do not align!\n    Site map <height, width, "
        "resolution, data.size()> <%ld, %ld, %f, %ld>\n    Design map <height, "
        "width, resolution, data.size()>: <%ld, %ld, %f, %ld>",
        current_height_map_.getHeight(), current_height_map_.getWidth(),
        current_height_map_.getResolution(),
        current_height_map_.getCellData().size(),
        design_height_map_.getHeight(), design_height_map_.getWidth(),
        design_height_map_.getResolution(),
  ...
  local_map_relative_to_global_frame_ =
      create_pose2d(xTransform, yTransform, 0.0);
  global_map_relative_to_local_frame_ =
      create_pose2d(-xTransform, -yTransform, 0.0);

  // ---------------------
  // DEBUG
  // global_robot_pose_ = cg::planning::create_pose2d(1.0, 1.0, 0.0);
  // current_agent_pose_ = cg::planning::transformPose(global_robot_pose_,
  // global_map_relative_to_local_frame_); // Convert pose to local map frame
  // ---------------------
}
...
void BehaviorExecutive::fsmTimerCallback() {
  ...
  case cg::planning::FSM::StateL0::UPDATE_MAP:
    ...
    RCLCPP_INFO(this->get_logger(), "Valid map update: %s",
                map_updated_ ? "true" : "false");
  ...
  case cg::planning::FSM::StateL0::SITE_WORK_DONE:
    site_work_done_.runState(current_height_map_, design_height_map_,
                             topology_equality_threshold_);
    break;
  case cg::planning::FSM::StateL0::MAP_EXPLORED:
    map_explored_.runState(current_map_coverage_ratio_,
                           map_coverage_threshold_);
    break;
  case cg::planning::FSM::StateL0::REPLAN_TRANSPORT:
    replan_transport_.runState();
    break;
  case cg::planning::FSM::StateL0::PLAN_TRANSPORT:
    plan_transport_.runState(*transport_planner_, current_height_map_,
                             design_height_map_, current_seen_map_,
                             transport_threshold_z_,
                             thresh_max_assignment_distance_);
    break;
  case cg::planning::FSM::StateL0::GET_TRANSPORT_GOALS:
    num_poses_before_ = current_goal_poses_.size(); // DEBUG
    get_transport_goals_.runState(current_goal_poses_, viz_state_l1_goal_poses_,
                                  *transport_planner_, current_agent_pose_,
                                  current_height_map_);
    // ---------------------------------------
    // DEBUG
    std::cout << "  Init / updated goal poses: " << num_poses_before_ << " / "
              << current_goal_poses_.size() << std::endl;
    std::cout << "    Agent <x,y,yaw>: < " << current_agent_pose_.pt.x << ", "
              << current_agent_pose_.pt.y << ", " << current_agent_pose_.yaw
              << " >" << std::endl;

    for (size_t i = 0; i < current_goal_poses_.size(); ++i) {
      std::cout << "    Pose " << i << " <x,y,yaw>: < "
                << current_goal_poses_[i].pt.x << ", "
                << current_goal_poses_[i].pt.y << ", "
                << current_goal_poses_[i].yaw << " >" << std::endl;
    }
    // ---------------------------------------
    break;
  ...
  case cg::planning::FSM::StateL0::GET_EXPLORATION_GOALS: {
    get_exploration_goals_.runState(
        current_goal_poses_, viz_state_l1_goal_poses_, *exploration_planner_,
        current_agent_pose_, current_height_map_);
    for (size_t i = 0; i < current_goal_poses_.size(); ++i) {
      std::cout << "    Exploration Pose <x,y,yaw>: " << std::to_string(i)
                << " < " << current_goal_poses_[i].pt.x << ", "
                << current_goal_poses_[i].pt.y << ", "
                << current_goal_poses_[i].yaw << " >" << std::endl;
    }
  ...
  case cg::planning::FSM::StateL0::GOALS_REMAINING: {
    ...
    std::cout << "    Current Goal Pose  <x,y,yaw>: < "
              << current_goal_pose_.pt.x << ", " << current_goal_pose_.pt.y
              << ", " << current_goal_pose_.yaw << " >" << std::endl;
    std::cout << "    Current Agent Pose <x,y,yaw>: < "
              << current_agent_pose_.pt.x << ", " << current_agent_pose_.pt.y
              << ", " << current_agent_pose_.yaw << " >" << std::endl;
    std::cout << "    ------------ Remaining goals: " << std::endl;
    for (size_t i = 0; i < current_goal_poses_.size(); ++i) {
      std::cout << "    Goal pose " << i << " <x,y,yaw>: < "
                << current_goal_poses_[i].pt.x << ", "
                << current_goal_poses_[i].pt.y << ", "
                << current_goal_poses_[i].yaw << " >" << std::endl;
    }
  ...
  case cg::planning::FSM::StateL0::GET_WORKSYSTEM_TRAJECTORY:
    // Get the trajectory
    // TODO: encapsulate these functions in to the state; e.g. make
    // GetWorksystemTrajectory a friend class of BehaviorExecutive so
    // GetWorksystemTrajectory can access service calls
    if (!calculated_trajectory_) {
      // Calculate path trajectory
      kinematic_planner_->generatePath(current_trajectory_.path,
                                       current_agent_pose_, current_goal_pose_,
                                       current_height_map_);

      // Calculate velocity trajectory
      velocity_planner_->generateVelocityTargets(
          current_trajectory_, current_agent_pose_, current_height_map_);

      // Calculate tool trajectory
      tool_planner_->enable(fsm_.getCurrStateL1() == FSM::StateL1::TRANSPORT);
      tool_planner_->generateToolTargets(
          current_trajectory_, current_agent_pose_, current_height_map_);

      // Convert to global frame
      for (unsigned int i = 0; i < current_trajectory_.path.size(); ++i) {
        cg_msgs::msg::Pose2D global_path_pose = cg::planning::transformPose(
            current_trajectory_.path[i], local_map_relative_to_global_frame_);
        current_trajectory_.path[i] = global_path_pose;
      }

      // DEBUG
      for (size_t i = 0; i < current_trajectory_.path.size(); ++i) {
        std::cout << "    Trajectory <x,y,yaw,v,tool>: " << std::to_string(i)
                  << " < " << current_trajectory_.path[i].pt.x << ", "
                  << current_trajectory_.path[i].pt.y << ", "
                  << current_trajectory_.path[i].yaw << ", "
                  << current_trajectory_.velocity_targets[i] << ", "
                  << current_trajectory_.tool_positions[i] << " >" << std::endl;
      }
    ...
    // Update shared current state and the precursing signal if worksystem is
    // now enabled
    get_worksystem_trajectory_.runState(
        worksystem_enabled_, updated_trajectory_, calculated_trajectory_);
    break;
  case cg::planning::FSM::StateL0::FOLLOWING_TRAJECTORY: {
    bool keep_following = following_trajectory_.runState(
        current_agent_pose_, current_goal_pose_, thresh_pos_, thresh_head_,
        thresh_euclidean_replan_, current_trajectory_, global_robot_state_,
        global_robot_pose_);
...
bool BehaviorExecutive::updateMapFromService(bool verbose = false) {
  ...
  // Send request
  if (verbose) {
    RCLCPP_INFO(this->get_logger(), "Sending SiteMap request");
  }
  ...
  if (status == std::future_status::ready) {
    if (verbose) {
      RCLCPP_INFO(this->get_logger(), "Received SiteMap response");
    }
    auto response = result_future.get();
    // Only update height map if data is valid
    if (response->success) {
      bool set_data =
          current_height_map_.setCellData(response->site_map.height_map);
    ...
    if (verbose) {
      RCLCPP_INFO(this->get_logger(), "No SiteMap response");
    }
    return false;
  }
}

bool BehaviorExecutive::updateTrajectoryService(
    const cg_msgs::msg::Trajectory &current_trajectory, bool verbose = false) {
  ...
  // Send request
  if (verbose) {
    RCLCPP_INFO(this->get_logger(), "Sending UpdateTrajectory request");
  }
  ...
  if (status == std::future_status::ready) {
    if (verbose) {
      RCLCPP_INFO(this->get_logger(), "Received UpdateTrajectory response");
    }
    auto response = result_future.get();
    ...
    if (verbose) {
      RCLCPP_INFO(this->get_logger(), "No UpdateTrajectory response");
    }
    return false;
  }
}

bool BehaviorExecutive::enableWorksystemService(const bool enable_worksystem,
                                                bool verbose = false) {
  ...
  // Send request
  if (verbose) {
    RCLCPP_INFO(this->get_logger(), "Sending EnableWorksystem request");
  }
  ...
  if (status == std::future_status::ready) {
    if (verbose) {
      RCLCPP_INFO(this->get_logger(), "Received EnableWorksystem response");
    }
    auto response = result_future.get();
    // Report response status
    ...
    if (verbose) {
      RCLCPP_INFO(this->get_logger(), "No EnableWorksystem response");
    }
    return false;
  }
}

void BehaviorExecutive::globalRobotStateCallback(
  ...
  global_robot_pose_ = cg::planning::create_pose2d(
      global_robot_state_.pose.pose.position.x,
      global_robot_state_.pose.pose.position.y, global_robot_yaw);

  // Convert pose to local map frame
  current_agent_pose_ = cg::planning::transformPose(
      global_robot_pose_, global_map_relative_to_local_frame_);
}

void BehaviorExecutive::odomRobotStateCallback(
    const nav_msgs::msg::Odometry::SharedPtr msg) {
...
void BehaviorExecutive::vizTimerCallback() {
  // Agent
  cg_msgs::msg::Pose2D global_agent_pose = cg::planning::transformPose(
      current_agent_pose_, local_map_relative_to_global_frame_);
  ...
  // Current goal
  cg_msgs::msg::Pose2D global_curr_goal_pose = cg::planning::transformPose(
      current_goal_pose_, local_map_relative_to_global_frame_);
  ...
  for (cg_msgs::msg::Pose2D goal_pose : current_goal_poses_) {
    cg_msgs::msg::Pose2D global_goal_pose = cg::planning::transformPose(
        goal_pose, local_map_relative_to_global_frame_);
  ...
  for (cg_msgs::msg::Pose2D goal_pose : viz_state_l1_goal_poses_) {
    ...
    pose_single.position.x = global_goal_pose.pt.x;
    pose_single.position.y = global_goal_pose.pt.y;
    pose_single.position.z =
        viz_planning_height_ *
        0.9; // Make L1 goals covered by current goals when overlapping
  ...
  for (cg_msgs::msg::Pose2D traj_pose : current_trajectory_.path) {
    // cg_msgs::msg::Pose2D global_traj_pose =
    // cg::planning::transformPose(traj_pose,
    // local_map_relative_to_global_frame_);
