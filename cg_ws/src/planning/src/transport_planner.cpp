...
void TransportPlanner::makeGoalsFromAssignment(
    const size_t assignment_idx, std::vector<cg_msgs::msg::Pose2D> &goalPoses) {

  // find desired heading of arg_min poses, equal to the arctan2 of the x & y
  // delta distances
  double yaw = static_cast<double>(
      atan2((transport_assignments_[assignment_idx].sink_node.y -
             transport_assignments_[assignment_idx].source_node.y),
            (transport_assignments_[assignment_idx].sink_node.x -
             transport_assignments_[assignment_idx].source_node.x)));

  // convert src, sink nodes to poses
  cg_msgs::msg::Pose2D source_pose = cg::planning::create_pose2d(
      transport_assignments_[assignment_idx].source_node.x,
      transport_assignments_[assignment_idx].source_node.y, yaw);
  cg_msgs::msg::Pose2D sink_pose = cg::planning::create_pose2d(
      transport_assignments_[assignment_idx].sink_node.x,
      transport_assignments_[assignment_idx].sink_node.y, yaw);
  ...
  cg_msgs::msg::Pose2D offset_pose =
      cg::planning::create_pose2d(offset_pose_x, offset_pose_y, yaw);
...
TransportPlanner::getGoalPose(const cg_msgs::msg::Pose2D &agent_pose,
  ...
  if ((transport_assignments_.size() == 0) ||
      (std::count(unvisited_assignments_.begin(), unvisited_assignments_.end(),
                  true) == 0)) {
    return goalPoses;
  }
  ...
  for (size_t i = 0; i < transport_assignments_.size(); ++i) {
    ...
    if (unvisited_assignments_.at(i)) {
      ...
      cg_msgs::msg::Point2D src_point = cg::planning::create_point2d(
          transport_assignments_.at(i).source_node.x,
          transport_assignments_.at(i).source_node.y);
...
/**
 * @brief Helper to initialize source and sink nodes along with respective
 * volumes
 *
 ...
 * @param current_height_map The current height map that should be modified by
 * the robot
 * @param design_height_map The desired final height map
 * @param threshold_z Symmetric band threshold for how different a current
 * height needs to be from design height to create a source or sink node
 */
void TransportPlanner::init_nodes(
    std::vector<TransportNode> &source_nodes,
    std::vector<TransportNode> &sink_nodes, float &vol_source, float &vol_sink,
    const cg::mapping::Map<float> &current_height_map,
    const cg::mapping::Map<float> &design_height_map,
    const std::vector<int> &seen_map, const float threshold_z) {

  // Loop through height map and assign points to either source or sink
  size_t num_cells =
      current_height_map.getHeight() * current_height_map.getWidth();
  for (size_t i = 0; i < num_cells; i++) {
    ...
    // Negative height becomes a sink; for solver the sinks also must have
    // positive volume, defined as positive in -z
...
/**
 * @brief Helper to calculate vector representing distance matrix between source
 * and sink nodes
 *
 * @param distances_between_nodes Empty container of distance values to be
 * populated
 * @param source_nodes Filled container of source nodes
 * @param sink_nodes Filled container of sink nodes
 */
void TransportPlanner::calculate_distances(
    std::vector<float> &distances_between_nodes,
    const std::vector<TransportNode> &source_nodes,
    const std::vector<TransportNode> &sink_nodes) {
  ...
  for (size_t i = 0; i < source_nodes.size(); i++) {
    for (size_t j = 0; j < sink_nodes.size(); j++) {
      ...
      cg_msgs::msg::Point2D source_pt =
          create_point2d(source_nodes[i].x, source_nodes[i].y);
      cg_msgs::msg::Point2D sink_pt =
          create_point2d(sink_nodes[j].x, sink_nodes[j].y);
...
/**
 * @brief Helper to solve optimization problem for finding transport plan and
 * the non-zero transport assignments.
 *
 * Additional notes:
 * - This method also updates new_transport_assignments with the non-zero
 * source-to-sink transports from the optimization solution.
 * - The source and sink volumes are used to calculate the binary decision
 * variable.
 *
 * @param new_transport_assignments Container of transport assignments to be
 * populated; will be cleared before population
 * @param source_nodes Filled container of source nodes
 * @param sink_nodes Filled container of sink nodes
 * @param distances_between_nodes Filled container of distance values, expected
...
float TransportPlanner::solveForTransportAssignments(
    ...
    const std::vector<TransportNode> &source_nodes,
    const std::vector<TransportNode> &sink_nodes,
    const std::vector<float> &distances_between_nodes, const float vol_source,
    const float vol_sink, const float thresh_max_assignment_distance,
    bool verbose = false) {
  // Declare the solver.
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  ...
  for (size_t t = 0; t < num_cells_policy; t++) {
    objective->SetCoefficient(transport_plan.at(t),
                              distances_between_nodes.at(t));
  }
  ...
  for (size_t j = 0; j < m_policy; j++) {
    ...
    constraints_sink_lb.push_back(
        solver->MakeRowConstraint(-infinity, sink_nodes.at(j).height));
  ...
  for (size_t i = 0; i < n_policy; i++) {
    // create a constraint for each sink
    constraints_source_lb.push_back(
        solver->MakeRowConstraint(-infinity, source_nodes.at(i).height));
    for (size_t j = 0; j < m_policy; j++) {
      // want all the rows
      size_t index = ij_to_index(i, j, m_policy);
  ...
  for (size_t j = 0; j < m_policy; j++) {
    // create a constraint for each sink
    constraints_sink_milp.push_back(solver->MakeRowConstraint(
  ...
  for (size_t i = 0; i < n_policy; i++) {
    // create a constraint for each sink
    constraints_source_milp.push_back(solver->MakeRowConstraint(
        source_nodes.at(i).height - (M * b), infinity));
    for (size_t j = 0; j < m_policy; j++) {
      size_t index = ij_to_index(i, j, m_policy);
      constraints_source_milp.at(i)->SetCoefficient(transport_plan.at(index),
                                                    1);
    }
  }
  ...
  for (size_t i = 0; i < n_policy; i++) {
    for (size_t j = 0; j < m_policy; j++) {
      ...
      // All non-zero transports that are close enough should be added as
      // assignments
      cg_msgs::msg::Point2D source_pt =
          create_point2d(source_nodes[i].x, source_nodes[i].y);
      cg_msgs::msg::Point2D sink_pt =
      ...
      if ((transport_volume > 0) &&
          (cg::planning::euclidean_distance(source_pt, sink_pt) <=
           thresh_max_assignment_distance)) {
        TransportAssignment new_assignment = {.source_node = source_nodes[i],
  ...
  unvisited_assignments_.resize(
      new_transport_assignments.size(),
      true); // update to correct size of transport plan, setting all true
  ...
  if (verbose) {
    ...
    for (size_t t = 0; t < num_cells_policy; t++) {
      LOG(INFO) << varNames.at(t) << ": "
                << transport_plan.at(t)->solution_value();
    }
  }
...
/**
 * @brief Primary method to plan optimal transport of volume from source nodes
 * to sink nodes
 *
 * Assumptions:
 ...
 */
float TransportPlanner::planTransport(
    const cg::mapping::Map<float> &current_height_map,
    const cg::mapping::Map<float> &design_height_map,
    const std::vector<int> &seen_map, const float threshold_z,
    const float thresh_max_assignment_distance) {
  // ---------------------------------------------------
  ...
  float vol_sink = 0.0f;
  init_nodes(source_nodes, sink_nodes, vol_source, vol_sink, current_height_map,
             design_height_map, seen_map, threshold_z);
  ...
  std::vector<TransportAssignment> new_transport_assignments;
  float objective_value = solveForTransportAssignments(
      new_transport_assignments, source_nodes, sink_nodes,
      distances_between_nodes, vol_source, vol_sink,
      thresh_max_assignment_distance);
...
/**
 * @brief Toy problem to verify optimization implementation; solution should
 * match write-up derivation and Python prototype
 *
 * Additional notes:
 * - Adjust the verbose variable in the method to toggle printing more detailed
 * log/solution info
 *
 * @return float The optimization objective value
 */
float TransportPlanner::solveToyProblem() {
  ...
  float objective_value = solveForTransportAssignments(
      new_transport_assignments, source_nodes, sink_nodes,
      distances_between_nodes, vol_source, vol_sink, verbose);
