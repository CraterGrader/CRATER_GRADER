...
void KinematicPlanner::generatePath(std::vector<cg_msgs::msg::Pose2D> &path,
  ...
  std::vector<std::vector<cg_msgs::msg::Pose2D>> base_lattice =
      KinematicPlanner::generateBaseLattice();

  // Run astar search
  path = KinematicPlanner::latticeAStarSearch(agent_pose, goal_pose, map,
                                              base_lattice);

  // Note: Path validity is already checked at every step of A* Search, do we
...
std::vector<cg_msgs::msg::Pose2D> KinematicPlanner::latticeAStarSearch(
    const cg_msgs::msg::Pose2D &agent_pose,
    const cg_msgs::msg::Pose2D &goal_pose, const cg::mapping::Map<float> &map,
  ...
  std::priority_queue<std::pair<float, AStarNode>,
  ...
  while (cur_equality_scalar < max_pose_equality_scalar_ && !found_plan) {
    ...
    visited_nodes.clear();
    pq_nodes = std::priority_queue<std::pair<float, AStarNode>,
                                   std::vector<std::pair<float, AStarNode>>,
                                   std::greater<std::pair<float, AStarNode>>>();
    ...
    while (!pq_nodes.empty()) {
      ...
      auto [closest_traj_pose, closest_traj_idx] =
          KinematicPlanner::getClosestTrajectoryPoseToGoal(curr_node.trajectory,
                                                           goal_pose);
      if (KinematicPlanner::samePoseWithinThresh(
        ...
        std::vector<cg_msgs::msg::Pose2D> traj_copy =
            visited_trajectories[curr_idx];
        std::reverse(traj_copy.begin(), traj_copy.end());
        final_path.insert(final_path.end(),
                          traj_copy.begin() +
                              (traj_copy.size() - 1 - closest_traj_idx),
                          traj_copy.end());
        ...
        while (curr_idx != 0) {
          ...
          std::vector<cg_msgs::msg::Pose2D> traj_copy =
              visited_trajectories[curr_idx];
          std::reverse(traj_copy.begin(), traj_copy.end());
          final_path.insert(final_path.end(), traj_copy.begin(),
                            traj_copy.end());
        ...
        for (cg_msgs::msg::Pose2D pose : final_path) {
          std::cout << "final path pose <x,y,yaw> : < " << pose.pt.x << ", "
                    << pose.pt.y << ", " << pose.yaw << std::endl;
        }
      ...
      std::vector<float> trajectories_heuristic =
          KinematicPlanner::trajectoriesHeuristic(valid_trajectories,
                                                  goal_pose);
      ...
      for (auto trajectory : valid_trajectories) {
        trajectories_costs.push_back(
            KinematicPlanner::calculateTopographyCost(trajectory, map));
      }

      // Calculate next nodes on all valid trajectories
      for (size_t traj_idx = 0; traj_idx < valid_trajectories.size();
           ++traj_idx) {

        // Skip if trajectory includes points out of bounds
        if (!KinematicPlanner::isValidTrajectory(valid_trajectories[traj_idx],
                                                 map))
          continue;

        cg_msgs::msg::Pose2D end_of_cur_traj_pose =
            valid_trajectories[traj_idx].back();
        ...
        for (auto visited_node : visited_nodes) {
          if (KinematicPlanner::samePoseWithinThresh(end_of_cur_traj_pose,
                                                     visited_node.pose)) {
            skip_new_node = true;
            break;
          }
        ...
        float succ_g_cost = max_trajectory_length_ + curr_node.g_cost +
                            trajectories_costs[traj_idx];
  ...
  if (final_path.empty()) {
    std::cout << "[WARN] No Solution Found, returning empty path..."
              << std::endl;
  }
...
bool KinematicPlanner::samePoseWithinThresh(
  ...
  if (end_pose_yaw < 0.0f)
    end_pose_yaw += 2.0 * M_PI;

  float goal_pose_yaw = std::fmod(goal_pose.yaw, 2.0 * M_PI);
  if (goal_pose_yaw < 0.0f)
    goal_pose_yaw += 2.0 * M_PI;

  return (euclidean_distance(trajectory_end_pose.pt, goal_pose.pt) <=
              pose_position_equality_threshold_ &&
          abs(end_pose_yaw - goal_pose_yaw) <= pose_yaw_equality_threshold_);
}

bool KinematicPlanner::samePoseWithinThresh(
    const cg_msgs::msg::Pose2D &trajectory_end_pose,
    const cg_msgs::msg::Pose2D &goal_pose, const float pose_threshold,
...
std::vector<std::vector<cg_msgs::msg::Pose2D>>
KinematicPlanner::generateBaseLattice() const {
...
KinematicPlanner::generateLatticeArm(float turn_radius, bool forwards,
                                     bool right) const {

  // Since left/right is handled with flags, turn radius should always be
  // positive
...
std::pair<cg_msgs::msg::Pose2D, int>
KinematicPlanner::getClosestTrajectoryPoseToGoal(
...
std::vector<std::vector<cg_msgs::msg::Pose2D>>
KinematicPlanner::transformLatticeToPose(
