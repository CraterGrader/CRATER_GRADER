...
public:
  ...
  KinematicPlanner()
      : goal_pose_distance_threshold_(0.15f),
        goal_pose_yaw_threshold_(deg2rad(5)), turn_radii_min_(1.6f),
        turn_radii_max_(2.8f), turn_radii_resolution_(0.4f),
        max_trajectory_length_(0.4f), trajectory_resolution_(0.05f),
        pose_position_equality_threshold_(0.05f),
        pose_yaw_equality_threshold_(deg2rad(5)), topography_weight_(1.0f),
        trajectory_heuristic_epsilon_(1.0f), max_pose_equality_scalar_(2.0),
  ...
  KinematicPlanner(float goal_pose_distance_threshold,
                   float goal_pose_yaw_threshold, float turn_radii_min,
                   float turn_radii_max, float turn_radii_resolution,
                   float max_trajectory_length, float trajectory_resolution,
                   float pose_position_equality_threshold,
                   float pose_yaw_equality_threshold, float topography_weight,
      ...
      : goal_pose_distance_threshold_(goal_pose_distance_threshold),
        ...
        turn_radii_min_(turn_radii_min), turn_radii_max_(turn_radii_max),
  ...
  void generatePath(std::vector<cg_msgs::msg::Pose2D> &path,
  ...
  bool samePoseWithinThresh(const cg_msgs::msg::Pose2D &trajectory_end_pose,
                            const cg_msgs::msg::Pose2D &goal_pose) const;
  ...
  bool isValidTrajectory(const std::vector<cg_msgs::msg::Pose2D> &trajectory,
                         const cg::mapping::Map<float> &map) const;

  // Calculates the total cost of the topography for trajectory
  float
  calculateTopographyCost(const std::vector<cg_msgs::msg::Pose2D> &trajectory,
                          const cg::mapping::Map<float> &map) const;
  ...
  generateLatticeArm(float turn_radius, bool forwards, bool right) const;
  ...
  float getPosePositionEqualityThreshold() {
    return pose_position_equality_threshold_;
  }
  float getPoseYawEqualityThreshold() { return pose_yaw_equality_threshold_; }
  float getTopographyWeight() { return topography_weight_; }
  float getTrajectoryHeuristicEpsilon() {
    return trajectory_heuristic_epsilon_;
  }
...
struct AStarNode {
  ...
  AStarNode(float g_cost, int idx, int parent_idx, cg_msgs::msg::Pose2D pose,
            std::vector<cg_msgs::msg::Pose2D> trajectory)
      : g_cost(g_cost), idx(idx), parent_idx(parent_idx), pose(pose),
        trajectory(trajectory){};

  bool operator<(const AStarNode &rhs) const { return g_cost < rhs.g_cost; }
};
