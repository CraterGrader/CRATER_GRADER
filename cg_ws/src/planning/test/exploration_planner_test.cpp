...
TEST(ExplorationPlannerTest, getGoalPoseTest) {
  std::unique_ptr<cg::planning::ExplorationPlanner> ep =
      std::make_unique<cg::planning::ExplorationPlanner>(
  ...
  double expected_squared_radius =
      (exploration_waypoints[0].pt.x - center_x) *
          (exploration_waypoints[0].pt.x - center_x) +
      (exploration_waypoints[0].pt.y - center_y) *
          (exploration_waypoints[0].pt.y - center_y);
  for (unsigned long i = 0; i < exploration_waypoints.size(); ++i) {
    ...
    if (i < exploration_waypoints.size() - 2) {
      ...
      EXPECT_NEAR((exploration_waypoints[i].pt.x - center_x) *
                          (exploration_waypoints[i].pt.x - center_x) +
                      (exploration_waypoints[i].pt.y - center_y) *
                          (exploration_waypoints[i].pt.y - center_y),
