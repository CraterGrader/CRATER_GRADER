...
TEST(KinematicPlannerTest, Test_generatePathForward) {
  ...
  cg::mapping::Map<float> map{static_cast<size_t>(100),
                              static_cast<size_t>(100), static_cast<float>(0.1),
                              cells};
  ...
  EXPECT_EQ(static_cast<int>(path.size()),
            static_cast<int>(
                (1.0f / kinematic_planner.getTrajectoryResolution()) + 1));
  EXPECT_NEAR(path.back().pt.x, goal_pose.pt.x,
              kinematic_planner.getGoalPoseDistanceThreshold());
  EXPECT_NEAR(path.back().pt.y, goal_pose.pt.y,
              kinematic_planner.getGoalPoseDistanceThreshold());
  EXPECT_NEAR(path.back().yaw, goal_pose.yaw,
              kinematic_planner.getGoalPoseYawThreshold());
}
...
TEST(KinematicPlannerTest, Test_generatePathForwardMultiple) {
  ...
  EXPECT_EQ(static_cast<int>(path.size()),
            static_cast<int>(
                (2.0f / kinematic_planner.getTrajectoryResolution()) + 1));
  EXPECT_NEAR(path.back().pt.x, goal_pose.pt.x,
              kinematic_planner.getGoalPoseDistanceThreshold());
  EXPECT_NEAR(path.back().pt.y, goal_pose.pt.y,
              kinematic_planner.getGoalPoseDistanceThreshold());
...
//   std::vector<cg_msgs::msg::Pose2D> path;
//   cg_msgs::msg::Pose2D agent_pose{cg::planning::create_pose2d(5, 5, 0)};
//   cg_msgs::msg::Pose2D goal_pose{cg::planning::create_pose2d(6, 6,
//   cg::planning::deg2rad(30))};
...
//   EXPECT_NEAR(path.back().pt.x, goal_pose.pt.x,
//   kinematic_planner.getPosePositionEqualityThreshold());
//   EXPECT_NEAR(path.back().pt.y, goal_pose.pt.y,
//   kinematic_planner.getPosePositionEqualityThreshold());
//   EXPECT_NEAR(path.back().yaw, goal_pose.yaw,
//   kinematic_planner.getPoseYawEqualityThreshold());
...
//   kinematic_planner.setPosePositionEqualityThreshold(0.05);
//   kinematic_planner.setPoseYawEqualityThreshold(cg::planning::deg2rad(5));
//   kinematic_planner.setTurnRadiiResolutuion(0.4f); // Finer lattice
//   resolution

//   std::vector<cg_msgs::msg::Pose2D> path;
//   cg_msgs::msg::Pose2D agent_pose{cg::planning::create_pose2d(5, 5, 0)};
//   cg_msgs::msg::Pose2D goal_pose{cg::planning::create_pose2d(2, 2,
//   cg::planning::deg2rad(90))};
...
TEST(KinematicPlannerTest, Test_getClosestTrajectoryPoseToGoal) {
  ...
  std::vector<cg_msgs::msg::Pose2D> trajectory{
      cg::planning::create_pose2d(0, 0, 0), expected_closest,
  ...
  closest_traj =
      kinematic_planner.getClosestTrajectoryPoseToGoal(trajectory, goalPose);
...
TEST(KinematicPlannerTest, Test_generateLatticeArm) {
  ...
  std::vector<cg_msgs::msg::Pose2D> lattice_arm_staight_forward =
      kinematic_planner.generateLatticeArm(0, true, true);
  ...
  std::vector<cg_msgs::msg::Pose2D> lattice_arm_straight_backward =
      kinematic_planner.generateLatticeArm(0, false, true);
  ...
  std::vector<cg_msgs::msg::Pose2D> lattice_arm_right_forward =
      kinematic_planner.generateLatticeArm(1, true, true);

  EXPECT_EQ(static_cast<int>(lattice_arm_right_forward.size()), 10);
  EXPECT_NEAR(lattice_arm_right_forward[0].pt.x, cos(M_PI / 2 - 0.1),
              absolute_range);
  EXPECT_NEAR(lattice_arm_right_forward[0].pt.y, sin(M_PI / 2 - 0.1) - 1,
              absolute_range);

  EXPECT_NEAR(lattice_arm_right_forward[9].pt.x, cos(M_PI / 2 - 1),
              absolute_range);
  EXPECT_NEAR(lattice_arm_right_forward[9].pt.y, sin(M_PI / 2 - 1) - 1,
              absolute_range);

  // Back left lattice arm
  std::vector<cg_msgs::msg::Pose2D> lattice_arm_left_backwards =
      kinematic_planner.generateLatticeArm(1, false, false);

  EXPECT_EQ(static_cast<int>(lattice_arm_left_backwards.size()), 10);
  EXPECT_NEAR(lattice_arm_left_backwards[0].pt.x, cos(-M_PI / 2 - 0.1),
              absolute_range);
  EXPECT_NEAR(lattice_arm_left_backwards[0].pt.y, sin(-M_PI / 2 - 0.1) + 1,
              absolute_range);

  EXPECT_NEAR(lattice_arm_left_backwards[9].pt.x, cos(-M_PI / 2 - 1),
              absolute_range);
  EXPECT_NEAR(lattice_arm_left_backwards[9].pt.y, sin(-M_PI / 2 - 1) + 1,
              absolute_range);
}
...
TEST(KinematicPlannerTest, Test_generateBaseLattice) {
  ...
  std::vector<std::vector<cg_msgs::msg::Pose2D>> base_lattice =
      kinematic_planner.generateBaseLattice();
...
TEST(KinematicPlannerTest, Test_transformLatticeToPose) {
  ...
  cg::planning::KinematicPlanner kinematic_planner;
  cg_msgs::msg::Pose2D current_pose{
      cg::planning::create_pose2d(3, 3, M_PI / 4)};
  float absolute_range = 1e-6f;
  ...
  auto transformed_lattice =
      kinematic_planner.transformLatticeToPose(lattice, current_pose);
  ...
  // Last point
  EXPECT_NEAR(transformed_lattice[0][2].pt.x, 3 - 2 * sin(M_PI / 4),
              absolute_range);
  EXPECT_NEAR(transformed_lattice[0][2].pt.y, 3 + 2 * cos(M_PI / 4),
              absolute_range);
...
TEST(KinematicPlannerTest, Test_isValidTrajectory) {
  ...
  cg::planning::KinematicPlanner kinematic_planner;
  cg::mapping::Map<float> map{static_cast<size_t>(2), static_cast<size_t>(2),
                              static_cast<float>(1)};
  std::vector<cg_msgs::msg::Pose2D> trajectory;
...
TEST(KinematicPlannerTest, Test_calculateTopographyCost) {
  ...
  EXPECT_NEAR(cost_2, kinematic_planner.getTopographyWeight() * 10,
              absolute_range);
}
...
TEST(KinematicPlannerTest, Test_calculateTopographyCostObstacle) {
  ...
  cg::mapping::Map<float> map{static_cast<size_t>(4), static_cast<size_t>(4),
                              static_cast<float>(1), cells};
  ...
  auto cost_2 = kinematic_planner.calculateTopographyCost(trajectory_2, map);
  EXPECT_NEAR(cost_2, kinematic_planner.getTopographyWeight() * 13,
              absolute_range);
}
...
TEST(KinematicPlannerTest, Test_calculateTopographyCostSmallResolution) {
  ...
  auto cost_1 = kinematic_planner.calculateTopographyCost(trajectory_1, map);
  EXPECT_NEAR(cost_1, kinematic_planner.getTopographyWeight() * 0.05,
              absolute_range);
  ...
  EXPECT_NEAR(cost_2, kinematic_planner.getTopographyWeight() * 0.55f,
              absolute_range);
}
...
TEST(KinematicPlannerTest, Test_trajectoriesHeuristic) {
  ...
  auto trajectory_heuristic =
      kinematic_planner.trajectoriesHeuristic(trajectories, goal_pose);
